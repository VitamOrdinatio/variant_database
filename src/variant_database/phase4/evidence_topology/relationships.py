...
def _make_member_rows(
    *,
    ...
    for row in rows:
        ...
        identity_kind = (
            str(row.get("identity_kind") or summary.get("identity_kind") or "")
            if is_source_identity_set_member
            else ""
        )
        participant_role = (
            str(row.get("participant_role") or summary.get("participant_role") or "")
            if is_source_identity_set_member
            else ""
        )
        source_namespace = (
            str(row.get("source_namespace") or summary.get("source_namespace") or "")
            if is_source_identity_set_member
            else ""
        )
        source_identity_count = (
            str(row.get("source_identity_count") or summary.get("source_identity_count") or "")
            if is_source_identity_set_member
            else ""
        )
        lossiness_status = (
            str(row.get("lossiness_status") or "")
            if is_source_identity_set_member
            else ""
        )
        resolution_status = (
        ...
        source_identity_set_status = (
            str(row.get("source_identity_set_status") or "")
            if is_source_identity_set_member
            else ""
        )
